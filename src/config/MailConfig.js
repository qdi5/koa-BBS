...
export default async function send (sendInfo) {
  ...
  let transporter = nodemailer.createTransport({
    ...
    auth: {
      user: 'postmaster@wuzhe.online', // 阿里云企业邮箱账号
      pass: 'fc128610()', // 密码
    },
  });
  const url = 'http://qdi5.com'
  // send mail with defined transport object
  let info = await transporter.sendMail({
    from: '"认证邮件" <postmaster@wuzhe.online>', // sender address
    to: sendInfo.email, // list of receivers
    subject: sendInfo.user !== '' ? `你好开发者，${sendInfo.user}!注册码` : '注册码', // Subject line
    text: `您在慕课网前端全站课程中注册，您的邀请码是${sendInfo.code}，邀请码的过期时间:${sendInfo.expire}`, // plain text body
